...
func parse(input string) ([]printConfig, [][]int) {
	parts := strings.Split(input, "\n\n")

	return parsePrintConfig(parts[0]),
		parsePageConfig(parts[1])
}

func parsePrintConfig(input string) []printConfig {
	var (
		runes = []rune(input)
		rLen  = len(runes)
...
func parsePageConfig(input string) [][]int {
	var (
		runes = []rune(input)
		rLen  = len(runes)
